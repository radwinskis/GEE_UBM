...
### Modified UBM Model 2 Workflow - ET and Soil Water Profile as input

def Modified_2_UBM_Step_Function(current_image, previous_state_list):
    ...
    if legacy_inputs == True:
        ...
        Available_Water_Initial = precipitation.add(snowmelt).add(irrigation).add(Soil_Water_Profile_Beginning_of_Timestep)
        ...
        # 1) Calculate Available Water in mm of water
        Available_Water_Initial = precip_and_snowmelt.add(irrigation).add(Soil_Water_Profile_Beginning_of_Timestep)
    ...
    Available_Water_for_Recharge_Scenario_3 = ee.Image(0)
...
def ModifiedUBM2Run(model_ready_collection, start_date=None, end_date=None):
    """
    Initializes the necessary data and runs the Modified UBM 2 model (ET and Soil Water Profile as input) step function over the provided model-ready collection 
    ...
    soil_moisture_img = model_ready_collection.first().select('Soil_Water_Beginning_of_Current_Timestep')
    ...
    initial_state_image = ee.Image([
                                    ...
                                    soil_moisture_img.rename('Soil_Water_End_Of_Previous_Timestep')
    ...
    UBM_results_list = ee.List(model_ready_collection.iterate(Modified_2_UBM_Step_Function, initial_state_list)).slice(1) # Slice off the initial state image
